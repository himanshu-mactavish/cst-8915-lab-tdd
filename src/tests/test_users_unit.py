...
def test_add_user(test_app, monkeypatch):
    ...
    resp = client.post(
        "/users",
        data=json.dumps({"username": "michael", "email": "michael@algonquinlive.com"}),
        content_type="application/json",
    )
    data = json.loads(resp.data.decode())
    assert resp.status_code == 201
    assert "michael@algonquinlive.com was added!" in data["message"]
...
def test_add_user_invalid_json_keys(test_app, monkeypatch):
    ...
    resp = client.post(
        "/users",
        data=json.dumps({"email": "john@algonquinlive.com"}),
        content_type="application/json",
    )
    data = json.loads(resp.data.decode())
    assert resp.status_code == 400
...
def test_single_user(test_app, monkeypatch):
    def mock_get_user_by_id(user_id):
        return {
            "id": 1,
            "username": "jeffrey",
            "email": "jeffrey@algonquinlive.com",
    ...
    assert "jeffrey@algonquinlive.com" in data["email"]
...
def test_remove_user(test_app, monkeypatch):
        ...
        d.update(
            {
                ...
                "email": "remove-me@algonquinlive.com",
            }
        )
        return d
    ...
    assert "remove-me@algonquinlive.com was removed!" in data["message"]
...
def test_update_user(test_app, monkeypatch):
    ...
    def mock_get_user_by_id(user_id):
        d = AttrDict()
        d.update({"id": 1, "username": "me", "email": "me@algonquinlive.com"})
        return d
    ...
    resp_one = client.put(
        "/users/1",
        data=json.dumps({"username": "me", "email": "me@algonquinlive.com"}),
        content_type="application/json",
    )
    ...
    assert "me@algonquinlive.com" in data["email"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [1, {"email": "me@algonquinlive.com"}, 400, "Input payload validation failed"],
        [
            999,
            {"username": "me", "email": "me@algonquinlive.com"},
            404,
